    ...
    const fetchUserNote = async () => {
        try {
            if (current !== undefined) {
                const response = await NoteService.fetchUserNote(userStore.user.id, current)
        ...
        <>
            ...
            <StyledEditNotePage>
                ...
                <div style={{ display: 'flex', height: '50px', gap: '20px' }}>
                    <Button background="#56C999" onClick={editNote} disabled={nameError === '' && descriptionError === '' ? false : true}>
                        Сохранить
                    </Button>
